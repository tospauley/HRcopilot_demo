...
// --- Simulated async loaders (no React Query needed) ---
...
const fetchAuditLogs = async (branchId: string) => {
...
const TIMEZONES = [
  ...
  'Europe/Paris', 'Africa/Lagos', 'Asia/Dubai', 'Asia/Singapore',
];
...
const Branches: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchBranches().then(data => { setBranches(data); setIsListLoading(false); });
  }, []);

  // Load employees when tab changes
  useEffect(() => {
    if (!selectedBranchId || detailTab !== 'Employees') return;
    setIsEmployeesLoading(true);
    fetchBranchEmployees(selectedBranchId).then(data => { setEmployees(data); setIsEmployeesLoading(false); });
  }, [selectedBranchId, detailTab]);

  // Load audit logs when tab changes
  useEffect(() => {
    if (!selectedBranchId || detailTab !== 'Audit') return;
    setIsAuditLoading(true);
    fetchAuditLogs(selectedBranchId).then(data => { setAuditLogs(data); setIsAuditLoading(false); });
  }, [selectedBranchId, detailTab]);
  ...
  const handleEdit = (branch: Branch) => { setFormBranch(branch); setView('FORM'); };
  const handleCreate = () => { setFormBranch({ type: 'Regional', status: 'Active', is_hq: false, timezone: 'UTC' }); setView('FORM'); };
  const handleViewDetail = (id: string) => { setSelectedBranchId(id); setDetailTab('Overview'); setView('DETAIL'); };
  const handleArchive = (id: string) => {
    setTimeout(() => setBranches(prev => prev.filter(b => b.id !== id)), 400);
  };
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        <div className="flex justify-between items-center px-4">
          ...
          <button onClick={() => setView('LIST')} className="px-4 py-2 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 font-bold text-[10px] uppercase tracking-widest rounded-xl transition-all">Cancel</button>
        </div>
        ...
        <GlassCard className="!p-8 border-t-4 border-t-[#0047cc]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div>
                ...
                <input type="text" value={formBranch.name || ''} onChange={e => setFormBranch({...formBranch, name: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formBranch.code || ''} onChange={e => setFormBranch({...formBranch, code: e.target.value})}
                ...
                <div>
                  ...
                  <select value={formBranch.timezone || 'UTC'} onChange={e => setFormBranch({...formBranch, timezone: e.target.value})}
              ...
              <div>
                ...
                <textarea value={formBranch.address || ''} onChange={e => setFormBranch({...formBranch, address: e.target.value})} rows={3}
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['HQ', 'Regional', 'Satellite', 'Virtual'].map(t => (
                    <button key={t} onClick={() => setFormBranch({...formBranch, type: t as any})}
              ...
              <div className="p-6 bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-3xl space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setFormBranch({...formBranch, is_hq: !formBranch.is_hq})}
          ...
          <div className="flex justify-end gap-4 mt-12 pt-8 border-t border-slate-100 dark:border-white/5">
            ...
            <button onClick={() => setView('LIST')} className="px-12 py-3.5 bg-emerald-500 text-white font-black text-[11px] uppercase tracking-widest rounded-2xl shadow-xl shadow-emerald-500/20 active:scale-95 transition-all">Deploy Configuration</button>
          </div>
        </GlassCard>
      </div>
    );
      ...
      <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-1 italic">Managed by {selectedBranch.manager_name}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">{selectedBranch.type} Hub</p>
                  </div>
                </div>
            ...
            <GlassCard title="Engagement Today" className="!p-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-center">
                  ...
                  <p className="text-[8px] font-black text-slate-500 uppercase">Present</p>
                </div>
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-center">
                  ...
                  <p className="text-[8px] font-black text-slate-500 uppercase">Late</p>
                </div>
              </div>
          ...
          <main className="lg:col-span-3 space-y-6">
            <div className="flex gap-2 border-b border-white/5 overflow-x-auto">
              {(['Overview', 'Employees', 'Devices', 'Overrides', 'Audit'] as const).map(t => (
            ...
            <div className="animate-in fade-in duration-500">
              {detailTab === 'Overview' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <GlassCard title="Workforce Distribution">
                    <div className="h-64 flex flex-col justify-center items-center">
                      <div className="w-40 h-40 rounded-full border-[8px] border-white/5 flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1">Employees</p>
                        </div>
                      </div>
                    </div>
                  </GlassCard>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                        <div className="flex gap-3 items-center">
                          ...
                          <div>
                            <p className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">Sync State</p>
                      ...
                      <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                        <div className="flex gap-3 items-center">
                          <div className="w-10 h-10 rounded-xl bg-[#0047cc]/10 flex items-center justify-center text-xl">🔐</div>
                          <div>
                            <p className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">MFA Compliance</p>
              ...
              {detailTab === 'Employees' && (
                <GlassCard className="!p-0 overflow-hidden">
                  ...
                  {isEmployeesLoading ? (
                    <div className="p-12 text-center animate-pulse text-slate-500 uppercase text-[10px] font-black italic">Synchronizing Employees...</div>
                  ) : (
              ...
              {detailTab === 'Devices' && (
                <div className="space-y-6">
                  <div className="flex justify-between items-center px-1">
                    <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase italic tracking-tight">Biometric Endpoints</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-5 bg-white/[0.02] border border-white/10 rounded-[28px] flex items-center gap-5 hover:bg-white/[0.04] transition-all">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl bg-emerald-500/10 text-emerald-400">📷</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight truncate">Face-ID LOBBY-01</p>
                        <p className="text-[9px] font-mono text-slate-500 mt-1 uppercase">SN: HRcopilot-SFO-901</p>
                      </div>
                    ...
                    <div className="p-5 bg-white/[0.02] border border-white/10 rounded-[28px] flex items-center gap-5 opacity-40 grayscale">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl bg-rose-500/10 text-rose-500">⚠️</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight truncate">Finger-Scan BACK-02</p>
                        <p className="text-[9px] font-mono text-slate-500 mt-1 uppercase">SN: HRcopilot-SFO-404</p>
                      </div>
                      <div className="text-right">
              ...
              {detailTab === 'Overrides' && (
                <GlassCard title="Regional Policy Overrides">
                  <div className="space-y-8 max-w-xl py-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">Grace Period Variance</p>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-white/5">
                      <div>
                        <p className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">Regional Holiday Calendar</p>
              ...
              {detailTab === 'Audit' && (
                <GlassCard className="!p-0 overflow-hidden">
                  {isAuditLoading ? (
                    <div className="p-12 text-center text-slate-500 uppercase text-[10px] font-black italic">Synchronizing Logs...</div>
                  ) : (
                      ...
                      {auditLogs.map((log: any) => (
                        <div key={log.id} className="p-5 flex justify-between items-center hover:bg-white/[0.01] transition-all">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">{log.event}</p>
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase italic">
            Distributed <span className="text-[#0047cc]">Infrastructure</span>
          </h2>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
            <button onClick={() => setView('LIST')} className={`p-2 rounded-xl transition-all ${view === 'LIST' ? 'bg-[#0047cc] text-white' : 'text-slate-500 hover:text-white'}`}><ListIcon size={18} /></button>
            <button onClick={() => setView('MAP')}  className={`p-2 rounded-xl transition-all ${view === 'MAP'  ? 'bg-[#0047cc] text-white' : 'text-slate-500 hover:text-white'}`}><MapIcon  size={18} /></button>
          </div>
          <button onClick={handleCreate} className="flex-1 sm:flex-none px-5 py-2.5 bg-[#0047cc] text-white font-black text-[11px] uppercase tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all">
            + Provision New Hub
          </button>
        </div>
      </div>
      ...
      {view === 'MAP' ? (
        <div className="h-[400px] sm:h-[700px] w-full animate-in zoom-in-95 duration-500">
          <GeofenceMap
            center={filteredBranches.length === 1 && filteredBranches[0].latitude ? [filteredBranches[0].latitude, filteredBranches[0].longitude] : [7.5, 5.0]}
            zoom={filteredBranches.length === 1 ? 7 : 5}
            branches={filteredBranches.filter(b => b.latitude && b.longitude && !(b.latitude === 0 && b.longitude === 0)).map(b => ({ id: b.id, name: b.name, lat: b.latitude || 0, lng: b.longitude || 0, image: b.image }))}
          />
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1 space-y-4">
            <GlassCard title="Governance Filters" className="!p-4">
              <div className="space-y-6">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search code or name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] block mb-3 px-1">Status</label>
            ...
            <div className="rounded-[2rem] overflow-hidden border border-slate-200 dark:border-white/10 relative" style={{ height: '240px' }}>
              <GeofenceMap
                ...
                center={filteredBranches.length === 1 && filteredBranches[0].latitude ? [filteredBranches[0].latitude as number, filteredBranches[0].longitude as number] : [7.5, 5.0]}
                zoom={filteredBranches.length === 1 ? 7 : 5}
                branches={filteredBranches.filter(b => b.latitude && b.longitude && !(b.latitude === 0 && b.longitude === 0)).map(b => ({ id: b.id, name: b.name, lat: b.latitude as number, lng: b.longitude as number, image: b.image }))}
              />
              <div className="absolute bottom-2 left-2 z-[999] bg-black/60 backdrop-blur-sm rounded-lg px-2.5 py-1 flex items-center gap-1.5 pointer-events-none">
                ...
                <span className="text-[8px] font-black text-white uppercase tracking-widest">{filteredBranches.filter(b => b.latitude && b.latitude !== 0).length} nodes</span>
              </div>
            </div>
          </aside>
          ...
          <main className="lg:col-span-3">
            <GlassCard className="!p-0 overflow-hidden border-white/5">
                        ...
                        <tr key={branch.id} className="group hover:bg-white/[0.03] transition-all cursor-pointer" onClick={() => handleViewDetail(branch.id)}>
                          <td className="px-6 py-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
                                  {branch.name}
                          ...
                          <td className="px-6 py-6 text-right pr-8">
                            <div className="flex justify-end gap-3" onClick={e => e.stopPropagation()}>
                              <button onClick={() => handleEdit(branch)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-400 hover:text-white hover:bg-white/10 transition-all">
                              ...
                              <button onClick={() => handleArchive(branch.id)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-all opacity-0 group-hover:opacity-100">
              ...
              <div className="p-5 border-t border-white/5 bg-white/[0.01] flex justify-between items-center text-[10px] font-black text-slate-500 uppercase tracking-widest italic">
                ...
                <span>Global Capacity: {branches.reduce((s, b) => s + b.employee_count, 0)} / 500</span>
              </div>
            </GlassCard>
          </main>
        </div>
